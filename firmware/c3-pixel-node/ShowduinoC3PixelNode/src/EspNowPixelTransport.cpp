...
static PixelNodeRxFn sHandler = nullptr;
...
static void onRx(const uint8_t *macAddr, const uint8_t *data, int len) {
  ...
  if (strcmp(pkt.nodeType, SHOWDUINO_LEGACY_NODETYPE_PIXEL) != 0 &&
      strcmp(pkt.nodeType, SHOWDUINO_PIXEL_NODE_TYPE) != 0) {
    sRej++;
    return;
  }
  ...
  if (src) {
    ...
    if (first || !sLoggedComms) {
      sLoggedComms = true;
      SD_LOGI("PIXEL", "COMMS CONNECTED");
    }
  }
  sRx++;
  sLastRx = millis();
  SD_LOGT("PIXEL", "RX seq=%lu cmd=%s", (unsigned long)pkt.sequence, pkt.command);
...
void pixelEspNowRecover() {
  /* Do not deinit ESP-NOW. Reassert channel and peers only. */
  pixelEspNowReassert();
}
...
bool pixelEspNowReady() { return sReady; }
void pixelEspNowSetHandler(PixelNodeRxFn fn) { sHandler = fn; }

bool pixelEspNowSend(const char *command, uint32_t sequence) {
  if (!sReady || !command || !command[0]) return false;
  ShowduinoNodePacket pkt = {};
  showduino_node_packet_init(&pkt, SHOWDUINO_LEGACY_NODETYPE_PIXEL, sequence);
...
uint32_t pixelEspNowRxCount() { return sRx; }
uint32_t pixelEspNowTxCount() { return sTx; }
