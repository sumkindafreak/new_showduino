...
| Safety | Current baseline Locate gesture is 8 presses in 6 seconds | P4 `BoardConfig.h` | `src/EmergencyInput.cpp` | `EmergencyInput.cpp` | YES — CODE | NO | **Not approved production requirement.** Gap 001. Do not present as final behaviour. |
| Safety | Approved 8-second continuous-hold Locate | Product-owner requirement for this manual pass | `SHOWDUINO_MANUAL_GAPS.md` | not implemented at baseline | NO — PLANNED | NO | Must not be documented as current behaviour. |
| Safety | Current baseline 3-second hold requests Emergency clearance | P4 `BoardConfig.h` | `EmergencyInput.cpp` | P4 input service | YES — CODE | HARDWARE REQUIRED | Conflicts with approved future 8-second Locate. Gap 003. |
...
| Locate | Current `DIRECTOR:LOCATE` wakes display and starts ambient locator | Director `.ino` | `DirectorAmbientPixels.cpp` | active Director | YES — CODE | HARDWARE REQUIRED | Current locator times out after 15 s. |
| Locate | Locate remains active until first deliberate touch, displays LOCATE ACTIVE, suspends auto-off | approved requirement | `SHOWDUINO_MANUAL_GAPS.md` | not implemented | NO — PLANNED | NO | Gap 002. |
...
| Emergency Node | Wireless Emergency Node is assert-only and cannot clear P4 emergency | `docs/emergency-node.md` | `docs/command-protocol.md` | `firmware/c3-emergency-node/` | YES — CODE | NO — GPIO UNCONFIRMED | Separate physical control model from main button. |
