...
/**
 * Showduino OS — cinematic LVGL 9 design system.
 * Presentation only: no protocol, runtime, or transport behaviour lives here.
 *
 * Layout contract:
 *   Status bar -> compact title -> compact summary -> scrollable content -> dock
 */

/* ---- Geometry ----------------------------------------------------------- */
#define OS_MARGIN            10
#define OS_GAP               7
#define OS_PANEL_RADIUS      14
#define OS_BTN_RADIUS        12
#define OS_PAD               10
#define OS_TITLE_H           36
#define OS_SUMMARY_H         58
#define OS_DOCK_Y            414
#define OS_DOCK_H            54
#define OS_CONTENT_LEFT_W    476
#define OS_CONTENT_RIGHT_X   500
#define OS_CONTENT_RIGHT_W   290
#define OS_CONTENT_FULL_W    (SCREEN_WIDTH - (2 * OS_MARGIN))

#define OS_BODY_Y            (6 + DirectorStatusBar::HEIGHT)
...
#define OS_DESK_SUMMARY_H    174
#define OS_DESK_ACTIONS_Y    (OS_BODY_Y + OS_DESK_SUMMARY_H + OS_GAP)
#define OS_DESK_ACTIONS_H    (OS_BODY_H - OS_DESK_SUMMARY_H - OS_GAP)
#define OS_ICON_SLOT_W       22

/* ---- Colour language ---------------------------------------------------- */
namespace OsColor {
  static const uint32_t Bg             = 0x02070B;
  static const uint32_t BgGlow         = 0x071B20;
  static const uint32_t Panel          = 0x071218;
  static const uint32_t PanelRaised    = 0x0B1D24;
  static const uint32_t PanelBorder    = 0x194957;
  static const uint32_t Button         = 0x0B2028;
  static const uint32_t ButtonBorder   = 0x247087;
  static const uint32_t ButtonPressed  = 0x123846;
  static const uint32_t Danger         = 0x3A0B10;
  static const uint32_t DangerBorder   = 0xFF4D5E;
  static const uint32_t Text           = 0xECFAFF;
  static const uint32_t TextMuted      = 0x75A0AD;
  static const uint32_t TextDim        = 0xA7C4CC;
  static const uint32_t Title          = 0xF5FDFF;
  static const uint32_t Ok             = 0x3CFFB0;
  static const uint32_t Warn           = 0xFFD166;
  static const uint32_t Fault          = 0xFF5A6A;
  static const uint32_t Unknown        = 0x55717A;
  static const uint32_t Accent         = 0x39E7FF;
  static const uint32_t AccentSoft     = 0x1999B2;
  static const uint32_t ScanLine       = 0x0D303A;
}
...
struct ShowduinoOsTheme {
  ...
  void begin() {
    ...
    lv_style_set_bg_grad_color(&screen, lv_color_hex(OsColor::BgGlow));
    ...
    lv_style_set_bg_opa(&panel, LV_OPA_90);
    ...
    lv_style_set_shadow_width(&panel, 12);
    lv_style_set_shadow_spread(&panel, 1);
    ...
    lv_style_set_bg_grad_color(&panelRaised, lv_color_hex(OsColor::Panel));
    lv_style_set_bg_grad_dir(&panelRaised, LV_GRAD_DIR_VER);
    ...
    lv_style_set_shadow_width(&panelRaised, 16);
    lv_style_set_shadow_spread(&panelRaised, 1);
    ...
    lv_style_set_bg_grad_color(&button, lv_color_hex(0x0D2A34));
    lv_style_set_bg_grad_dir(&button, LV_GRAD_DIR_VER);
    ...
    lv_style_set_shadow_width(&buttonPressed, 14);
    ...
    lv_style_set_bg_grad_color(&buttonDanger, lv_color_hex(0x64111B));
    lv_style_set_bg_grad_dir(&buttonDanger, LV_GRAD_DIR_VER);
    ...
    lv_style_set_pad_all(&buttonDanger, 9);
    lv_style_set_shadow_color(&buttonDanger, lv_color_hex(OsColor::DangerBorder));
    lv_style_set_shadow_width(&buttonDanger, 12);
    lv_style_set_shadow_opa(&buttonDanger, LV_OPA_30);

    lv_style_init(&buttonDangerPressed);
    lv_style_set_bg_color(&buttonDangerPressed, lv_color_hex(0x7A1722));
    lv_style_set_border_color(&buttonDangerPressed, lv_color_hex(0xFF8290));
    lv_style_set_shadow_color(&buttonDangerPressed, lv_color_hex(OsColor::DangerBorder));
    lv_style_set_shadow_width(&buttonDangerPressed, 16);
    ...
    lv_style_set_radius(&chip, 12);
    lv_style_set_pad_hor(&chip, 9);
    lv_style_set_pad_ver(&chip, 3);
    lv_style_set_text_color(&chip, lv_color_hex(OsColor::Accent));

    lv_style_init(&progressBg);
    lv_style_set_bg_color(&progressBg, lv_color_hex(0x050D12));
    ...
    lv_style_set_radius(&progressBg, 6);

    lv_style_init(&progressIndicator);
    lv_style_set_bg_color(&progressIndicator, lv_color_hex(OsColor::Accent));
    lv_style_set_bg_grad_color(&progressIndicator, lv_color_hex(OsColor::Ok));
    lv_style_set_bg_grad_dir(&progressIndicator, LV_GRAD_DIR_HOR);
    lv_style_set_bg_opa(&progressIndicator, LV_OPA_COVER);
    lv_style_set_radius(&progressIndicator, 6);
    lv_style_set_shadow_color(&progressIndicator, lv_color_hex(OsColor::Accent));
    lv_style_set_shadow_width(&progressIndicator, 10);
    lv_style_set_shadow_opa(&progressIndicator, LV_OPA_40);

    lv_style_init(&scrollbar);
    lv_style_set_bg_color(&scrollbar, lv_color_hex(OsColor::Accent));
    lv_style_set_bg_opa(&scrollbar, LV_OPA_50);
    lv_style_set_radius(&scrollbar, 3);
    lv_style_set_width(&scrollbar, 4);
  ...
  lv_obj_t *makeScrollablePanel(lv_obj_t *parent, int x, int y, int w, int h, bool raised = false) {
    lv_obj_t *p = makePanel(parent, x, y, w, h, raised);
    lv_obj_add_flag(p, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_scroll_dir(p, LV_DIR_VER);
    lv_obj_set_scrollbar_mode(p, LV_SCROLLBAR_MODE_AUTO);
    lv_obj_add_style(p, &scrollbar, LV_PART_SCROLLBAR);
    lv_obj_set_style_pad_right(p, OS_PAD + 4, 0);
    return p;
  }
  ...
  lv_obj_t *makePageTitle(lv_obj_t *parent, const char *text, int x = 8, int y = 4) {
    lv_obj_t *accent = lv_obj_create(parent);
    lv_obj_remove_style_all(accent);
    lv_obj_set_pos(accent, x, y + 1);
    lv_obj_set_size(accent, 4, 19);
    lv_obj_set_style_bg_color(accent, lv_color_hex(OsColor::Accent), 0);
    lv_obj_set_style_bg_opa(accent, LV_OPA_COVER, 0);
    lv_obj_set_style_radius(accent, 2, 0);
    lv_obj_set_style_shadow_color(accent, lv_color_hex(OsColor::Accent), 0);
    lv_obj_set_style_shadow_width(accent, 8, 0);
  ...
  lv_obj_t *makePageChrome(lv_obj_t *screen, const char *pageTitle, lv_obj_t **outTitleBar = nullptr) {
    ...
    makeChip(titleBar, "SHOWDUINO OS", OS_CONTENT_FULL_W - 154, -1);
  ...
  lv_obj_t *makePrimaryPanel(lv_obj_t *screen) {
    return makeScrollablePanel(screen, OS_MARGIN, OS_PRIMARY_Y, OS_CONTENT_FULL_W, OS_PRIMARY_H);
  }

  void makeDock(lv_obj_t *screen, lv_event_cb_t cb, void *user) {
    const int gap = OS_GAP;
    const int estopW = 132;
