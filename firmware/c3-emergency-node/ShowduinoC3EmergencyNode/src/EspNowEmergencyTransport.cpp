...
static EmergencyNodeRxFn sHandler = nullptr;
...
static void onRx(const uint8_t *macAddr, const uint8_t *data, int len) {
  ...
  if (strcmp(pkt.nodeType, SHOWDUINO_LEGACY_NODETYPE_EMERGENCY) != 0 &&
      strcmp(pkt.nodeType, SHOWDUINO_EMERGENCY_NODE_TYPE) != 0) {
    sRej++;
    return;
  }
  ...
  if (src) {
    ...
    if (first || !sLoggedComms) {
      sLoggedComms = true;
      SD_LOGI("ESTOP", "COMMS CONNECTED");
    }
  }
  sRx++;
  sLastRx = millis();
  SD_LOGT("ESTOP", "RX seq=%lu cmd=%s", (unsigned long)pkt.sequence, pkt.command);
...
bool emergencyEspNowReady() { return sReady; }
void emergencyEspNowSetHandler(EmergencyNodeRxFn fn) { sHandler = fn; }

bool emergencyEspNowSend(const char *command, uint32_t sequence) {
  if (!sReady || !command || !command[0]) return false;
  ShowduinoNodePacket pkt = {};
  showduino_node_packet_init(&pkt, SHOWDUINO_LEGACY_NODETYPE_EMERGENCY, sequence);
...
uint32_t emergencyEspNowRxCount() { return sRx; }
uint32_t emergencyEspNowTxCount() { return sTx; }
uint32_t emergencyEspNowRejected() { return sRej; }
uint32_t emergencyEspNowLastRxMs() { return sLastRx; }
bool emergencyEspNowHaveComms() { return sHaveComms; }
uint8_t emergencyEspNowChannel() { return sFollow.channel(); }
int8_t emergencyEspNowRssi() { return sRssi; }
