...
static void audioPixelEngineShutdown() {
  sLocateActive = false;
  sTestActive = false;
  if (sReady && sFrame) {
    clearFrame();
...
bool audioPixelEngineBegin() {
  ...
  if (sReady) audioPixelEngineShutdown();
...
bool audioPixelEngineReady() { return sReady; }
bool audioPixelEngineEmergency() { return sEmergency; }
bool audioPixelEngineLocateActive() { return sLocateActive; }
uint16_t audioPixelEngineCount() { return sCount; }
...
bool audioPixelEngineHandleCommand(const char *command, char *reply, size_t replyLen) {
  ...
  if (n == 3 && p[1] == "COUNT") {
    ...
    uint16_t count = 0;
    if (!parseU16(p[2], &count) || !showduino_pixel_count_ok(count, SHOWDUINO_AUDIO_PIXEL_MAX_PIXELS)) {
