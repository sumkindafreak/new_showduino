...
This is an interactive carbide-lamp practical (jewel flame, striker, blow-to-extinguish, local Fermion FX). Firmware **0.3.4** is both a standalone SoftAP carbide lamp and a managed Showduino node. It is not a Pixel Node and not a second Audio Node. Production GPIOs are confirmed. See [`s3-lamp-node.md`](s3-lamp-node.md).
