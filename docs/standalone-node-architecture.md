...
- S3 Lamp Node `0.4.0` — same GRANT machine; standalone carbide lamp + local WebUI; jewel flame experience (blackout, flint, catch, living flame); optional GPIO15 motion input (default DISABLED); V1 Fermion four-file library; physical GPIOs confirmed except motion sensor. Historical C3 Lamp Node remains reference-only.
