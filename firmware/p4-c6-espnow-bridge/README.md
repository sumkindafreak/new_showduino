...
Reserved-hardware diagnostics (not the live product path) remain here:

```text
firmware/p4-c6-espnow-bridge/arduino-hosted-link-qualification/
firmware/p4-c6-espnow-bridge/hosted-link-qualification/
```

Those sketches prove P4→SDIO→C6 Wi-Fi without replacing factory C6 firmware. Do not treat them as the Communications Engine.
