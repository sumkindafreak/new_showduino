...
function buildControlStrip(state) {
  ...
  const time = el('div', {
    ...
    text: '00:00.000',
    title: 'Authoritative playhead time reported by the P4 Stage Runtime.'
  });
  ...
  const clearEstop = button('CLEAR E-STOP', 'show-editor-btn', async () => {
    if (!window.confirm('Clear the Showduino emergency stop? The P4 will reject this while the physical button is still asserted.')) return;
    try {
...
async function refreshRuntime(state) {
  try {
    ...
    state.connectionBadge.textContent = sys.stageLink === 'offline' ? 'SUE ONLY' : 'ONLINE';
    state.connectionBadge.classList.remove('offline');
    state.connectionBadge.classList.add('online');

    if (sys.showName) state.projectName.textContent = sys.showName;
    if (sys.showElapsedMs != null) {
      state.timeMs = Number(sys.showElapsedMs) || 0;
      state.paintTime();
    }
...
export async function ShowsPage(container) {
  ...
  state.projectName = shell.querySelector('.show-editor-project-name');
