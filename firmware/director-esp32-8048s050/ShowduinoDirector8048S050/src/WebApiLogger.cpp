...
WebApiLogger::WebApiLogger() : buffer(nullptr), head(0), count(0), mux(portMUX_INITIALIZER_UNLOCKED) {
...
void WebApiLogger::appendJsonArray(String &out) {
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    out += ",\"source\":\"" + ShowduinoFileUtil::jsonEscape(String(e.source)) + "\"";
    out += ",\"message\":\"" + ShowduinoFileUtil::jsonEscape(String(e.message)) + "\"";
    out += '}';
  }
  portEXIT_CRITICAL(&mux);
  out += "\n]";
