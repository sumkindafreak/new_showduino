...
#define SHOWDUINO_LEGACY_SHOW_PAUSE       "SHOW:PAUSE"
#define SHOWDUINO_LEGACY_SHOW_RESUME      "SHOW:RESUME"
...
#define SHOWDUINO_LEGACY_PRODUCTION_LIST          "PRODUCTION:LIST"
#define SHOWDUINO_LEGACY_PRODUCTION_LOAD_PREFIX   "PRODUCTION:LOAD:"
#define SHOWDUINO_LEGACY_PRODUCTION_UNLOAD        "PRODUCTION:UNLOAD"
#define SHOWDUINO_LEGACY_PRODUCTION_STATUS        "PRODUCTION:STATUS"
...
#define SHOWDUINO_LEGACY_ERR_ESTOP_HELD   "ERR:EMERGENCY_CLEAR:BUTTON_ASSERTED"
...
static inline ShowduinoMessageType showduino_legacy_map_command(const char *cmd) {
  ...
  if (strcmp(cmd, SHOWDUINO_LEGACY_SHOW_PAUSE) == 0) return SHOWDUINO_MSG_SHOW_PAUSE_REQUEST;
  if (strcmp(cmd, SHOWDUINO_LEGACY_SHOW_RESUME) == 0) return SHOWDUINO_MSG_SHOW_RESUME_REQUEST;
  if (strcmp(cmd, SHOWDUINO_LEGACY_STOP_ALL) == 0) return SHOWDUINO_MSG_SHOW_STOP_REQUEST;
  if (strcmp(cmd, SHOWDUINO_LEGACY_PRODUCTION_LIST) == 0) {
    return SHOWDUINO_MSG_PRODUCTION_LIST_REQUEST;
  }
  if (strncmp(cmd, SHOWDUINO_LEGACY_PRODUCTION_LOAD_PREFIX,
              strlen(SHOWDUINO_LEGACY_PRODUCTION_LOAD_PREFIX)) == 0) {
    return SHOWDUINO_MSG_PRODUCTION_LOAD_REQUEST;
  }
  if (strcmp(cmd, SHOWDUINO_LEGACY_PRODUCTION_UNLOAD) == 0) {
    return SHOWDUINO_MSG_PRODUCTION_UNLOAD_REQUEST;
  }
  if (strcmp(cmd, SHOWDUINO_LEGACY_PRODUCTION_STATUS) == 0) {
    return SHOWDUINO_MSG_PRODUCTION_STATUS_REQUEST;
  }
