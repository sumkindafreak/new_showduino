 ...
 *
 * Keeps the existing TimelineEditor authoritative while making its first-use
 * experience quieter and easier to understand. This file changes presentation
 * and discoverability only; SHDO v2 and runtime behaviour are untouched.
 */
(function () {
  'use strict';
  ...
  function metaFor(type) {
    return TYPE_META[String(type || 'mixed').toLowerCase()] || TYPE_META.mixed;
  }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      @media (min-width:761px) {
        ...
        .timeline-editor.showduino-clarity-ready .timeline-main {
        ...
        .timeline-editor.showduino-clarity-ready .tl-track-header:hover {
        ...
        .tl-track-header.sd-tools-open .sd-lane-tools {
        ...
        .timeline-editor.showduino-clarity-ready .tl-track-row:nth-of-type(even) {
          background:#11191e !important;
        }
        ...
        .timeline-editor.showduino-clarity-ready .tl-marker-track {
          ...
          border-bottom-color:#26343b !important;
          opacity:.62;
        }

        .timeline-editor.showduino-clarity-ready .tl-canvas-scroll {
          background:#0c1215;
        }
        ...
        .timeline-editor.showduino-clarity-ready .tl-clip.sd-selected-cue {
          outline:2px solid #f2fbfb !important;
          outline-offset:1px;
        ...
        .sd-empty-state {
          ...
          padding:18px;
          border:1px dashed #344a53;
          border-radius:14px;
          background:rgba(13,21,25,.95);
        ...
        .timeline-editor.showduino-clarity-ready .tl-inspector {
          padding:0 !important;
          border-left:1px solid #293941 !important;
        ...
        .timeline-editor.showduino-clarity-ready .tl-inspector #insp-params label {
          ...
          font:750 10px/1.2 Inter,system-ui,sans-serif !important;
        }
        ...
        .timeline-editor.showduino-clarity-ready .tl-inspector select {
          min-height:34px;
          border:1px solid #32464f !important;
          border-radius:7px !important;
        ...
        .sd-inspector-advanced {
          display:none !important;
        }

        .tl-inspector.sd-show-advanced .sd-inspector-advanced {
        ...
        .timeline-toolbar.showduino-toolbar-simplified button.sd-quiet-tool {
          display:none !important;
        }
      }
    `;
  ...
  function getProjectTracks() {
    ...
    return editor && typeof editor._tracks === 'function' ? editor._tracks() : [];
  }

  function getProjectClips() {
    const editor = timeline();
    return editor && typeof editor._clips === 'function' ? editor._clips() : [];
  }
  ...
  function enhanceToolbar(editor) {
    ...
    toolbar.querySelectorAll('button').forEach((button) => {
      ...
      if (text === '⋮ Grid: ON' || text === '⋮ Grid: OFF' || text === '📌 Marker' || text === '＋ New') {
        button.classList.add('sd-quiet-tool');
      }
    });
  }
  ...
  function enhanceLaneHeaders(editor) {
    ...
    editor.querySelectorAll('.tl-track-header').forEach((header) => {
      const track = tracks.find((item) => item.id === header.dataset.trackId);
      if (!track) return;
    ...
    if (listHeader) listHeader.textContent = 'Show lanes';
  }
  ...
  function enhanceLaneRows(editor) {
    ...
    editor.querySelectorAll('.tl-track-row').forEach((row) => {
      ...
      const hasClip = clips.some((clip) => clip.trackId === track.id);
  ...
  function enhanceClips(editor) {
    ...
    editor.querySelectorAll('.tl-clip').forEach((element) => {
      ...
      if (label) {
        ...
        label.textContent = `${meta.icon} ${clip.label || meta.name}`;
      }
  ...
  function clickCueShelf(type) {
    const button = document.querySelector(`.showduino-cue-tool[data-cue-type="${type}"]`);
  ...
  function updateEmptyState(editor) {
    ...
    empty.innerHTML = `
      ...
      <p>Start with the first thing the audience should see or hear. You can drag an action from above, or use one of these shortcuts.</p>
  ...
  function friendlyInspectorLabel(label) {
    ...
    if (replacements[text]) label.textContent = replacements[text];
  }
  ...
  function enhanceInspector(editor) {
    ...
    const clip = getProjectClips().find((item) => item.id === selectedId);
    ...
    if (heading) {
      ...
      heading.textContent = 'Cue settings';
    ...
    const fields = Array.from(inspector.querySelectorAll('.inspector-field'));
    fields.forEach((field) => {
      const label = field.querySelector('label')?.textContent.trim();
      if (label === 'Timeline colour' || label === 'Lane') field.classList.add('sd-inspector-advanced');
    });
    ...
    if (!inspector.querySelector('.sd-inspector-more')) {
      ...
      more.addEventListener('click', () => {
        ...
        more.textContent = inspector.classList.contains('sd-show-advanced') ? 'Hide timeline settings' : 'More timeline settings';
      });
